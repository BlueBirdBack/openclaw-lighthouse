...
### 1) `doctor --repair` did not recover OpenClaw
...
### 2) Clean uninstall + reinstall (root/Linux)
...
### 3) Telegram setup after fresh install (especially root VPS)
...
### 4) New VPS install: gateway health check fails with `1006`
...
### 5) Root VPS: C3 down after plugin + restart failure
...
### 6) Bot replies to text, but not photos/files
...
### 7) `/approve` unauthorized or `/elevated full` blocked
...
### 8) OpenAI/Gemini required for memory?
...
### 9) Multi-instance fleet replies are slow or missing (`session file locked`)
...
### 10) New Telegram bot says `chat not found`
